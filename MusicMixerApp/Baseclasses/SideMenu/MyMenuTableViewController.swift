...
class MyMenuTableViewController: UITableViewController {
    let arrNames = ["Creation","Community","Stream","Shop"]
    ...
    override func viewDidLoad() {
        ...
        tableView.backgroundColor = UIColor.textColorBlack()
        tableView.scrollsToTop = false
        tableView.tableFooterView=UIView(frame: .zero)
        // Preserve selection between presentations
        self.clearsSelectionOnViewWillAppear = false
        tableView.separatorColor=UIColor.textMenuNormal()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (cell == nil) {
            ...
            cell!.backgroundColor = UIColor.textColorBlack()
            cell!.textLabel?.textColor = UIColor.textMenuNormal()
            let selectedBackgroundView = UIView(frame: CGRect(x: 0, y: 0, width: cell!.frame.size.width, height: cell!.frame.size.height))
            selectedBackgroundView.backgroundColor = UIColor.gray.withAlphaComponent(0.2)
            cell!.selectedBackgroundView = selectedBackgroundView
            cell?.textLabel?.font=AppFont.systemFontWithWeightAndSize(fontName: .Regular, size: 16)
        }
